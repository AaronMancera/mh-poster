    ...
    try {
      ...
      setGenError(
        err.response?.data?.error ||
          "Error al generar el póster. ¿Está corriendo la API?",
      );
    } finally {
      setGenerating(false);
    }
      ...
      <div className="detail-layout">
        ...
        <div className="detail-info">
          ...
          {monster.elements?.length > 0 && (
            <p>
              <strong>Elementos:</strong> {monster.elements.join(", ")}
            </p>
          )}
          {monster.weaknesses?.length > 0 && (
            <p>
              <strong>Debilidades:</strong> {monster.weaknesses.join(", ")}
            </p>
          )}
          {monster.games?.length > 0 && (
            <p>
              <strong>Aparece en:</strong> {monster.games.join(", ")}
            </p>
          )}
